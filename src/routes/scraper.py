...
def scan_wallapop(location, max_price):
    """Scan Wallapop for electronics under max_price"""
    results = []
    ...
    params = {
        ...
        'latitude': '37.9838',  # Murcia coordinates
        'longitude': '-1.1297',
    ...
    try:
        ...
        if response.status_code == 200:
            # Note: Wallapop uses dynamic loading, so this is a simplified approach
            # In a real implementation, you'd need to use Selenium or similar
            soup = BeautifulSoup(response.content, 'html.parser')
            
            # Look for item containers (this would need to be adjusted based on actual HTML structure)
            items = soup.find_all('div', class_='item-container')  # Placeholder selector
                        ...
                        if price_match:
                            ...
                            if price <= max_price:
                                results.append({
                                    ...
                                    'profit_potential': calculate_profit_potential(price, estimate_resale_value(title, price))
                                })
                except Exception as e:
                    continue
                    
    except Exception as e:
        print(f"Error fetching Wallapop data: {e}")
